...
function closeCalendar() {
	...
	ctlToPlaceValue.value	= date.substr(0, 4) + "-" + date.substr(4, 2) + "-" + date.substr(6, 2); 
}
...
function popUpCalendar(ctl, ctl2, format, num) {
	
	var leftpos = 0;
	var toppos = 0;

	if(bPageLoaded) {
		if(ctl2.length == undefined){
			ctlToPlaceValue	= ctl2;
		}
		else
		{
			ctlToPlaceValue	= ctl2[num];
		}
